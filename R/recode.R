...
#' don't exist or cannot be recreated using variables in Census. In some
...
#'
#'
#' @param x [data.frame()] A combined persons and household of VISTA 
#'  or Census population.
...
#' @export
#' @rdname morph_vista
vista_to_cvista <- function(x, cvista_only = FALSE, extra_cols = NULL) {
    checkmate::assert_data_frame(x)
    class(x) <- c("vista1218", class(x))
    morph_cvista(x, cvista_only, extra_cols)
}

#' @export
#' @rdname morph_vista
csf_to_cvista <- function(x, cvista_only = FALSE, extra_cols = NULL) {
    checkmate::assert_data_frame(x)
    class(x) <- c("csf2016", class(x))
    morph_cvista(x, cvista_only, extra_cols)
}
